...
from sqlalchemy import (
    ...
    Boolean, ForeignKey, event, select, text
)
...
# ─────────────────────────────────────────────────────────
# 환경 변수에서 DB 접속 정보 / 관리자/주방 계정 정보 로드
# ─────────────────────────────────────────────────────────
...
engine = create_engine(
    ...
    pool_timeout=30,
    pool_recycle=1800,
    echo=False  # 디버그용 True 설정 가능
)

SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
Base = declarative_base()

# Flask 요청 스레드마다 세션 분리
def get_db_session():
...
class Order(Base):
    ...
    id            = Column(Integer, primary_key=True, autoincrement=True)
    order_id      = Column(String(50), nullable=False)  # time.time()을 문자열로
    ...
    createdAt     = Column(Integer, nullable=False)     # epoch sec
    confirmedAt   = Column(Integer)                     # paid 시점
    alertTime1    = Column(Integer, nullable=False, default=0)  # 50분 알림
    alertTime2    = Column(Integer, nullable=False, default=0)  # 60분 알림
    service       = Column(Boolean, nullable=False, default=False)  # 0원 여부
...
class Log(Base):
    __tablename__ = "logs"
    id      = Column(Integer, primary_key=True, autoincrement=True)
    time    = Column(Integer, nullable=False)  # epoch sec
    ...
    def run_checker():
        while True:
            ...
            try:
                ...
                # paid 상태 주문들 (confirmedAt != None)
                orders_paid = db.query(Order).filter(Order.status=="paid", Order.confirmedAt!=None).all()
...
def admin():
    ...
    # paid
    paid_orders_raw = db.query(Order).filter_by(status="paid").order_by(Order.id.desc()).all()
    paid_orders = []
    for o in paid_orders_raw:
        items = []
        for it in o.items:
            mname = it.menu.name if it.menu else "?"
            items.append({
    ...
    # completed
    completed_orders_raw = db.query(Order).filter_by(status="completed").order_by(Order.id.desc()).all()
    completed_orders = []
...
def admin_deliver_item(order_id, menu_name):
    ...
    try:
        ...
        if delivered_item.deliveredQuantity < delivered_item.doneQuantity:
            delivered_item.deliveredQuantity += 1

        # 모든 item 전달완료 시 → completed
        all_delivered = True
        for it in o.items:
...
def kitchen():
    db = get_db_session()
    paid_orders_raw = db.query(Order).filter_by(status="paid").order_by(Order.confirmedAt.asc()).all()
    paid_orders = []
    item_count = {}
    ...
    for o in paid_orders_raw:
        ...
        for it in o.items:
            ...
            left = qty - done
            if left>0:
                item_count[menuname] = item_count.get(menuname,0) + left
        paid_orders.append({
            "id": o.id,
            "order_id": o.order_id,
            "tableNumber": o.tableNumber,
...
def kitchen_done_item(order_id, menu_name):
    ...
    try:
        ...
        if target_item.doneQuantity < target_item.quantity:
            target_item.doneQuantity += 1
