...
# ─────────────────────────────────────────────────────────
# AWS MySQL 접속 정보 & 관리자/주방 계정 (절대 수정 금지)
# ─────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────
# 에러 핸들러 (구체적 예외를 터미널에 출력)
# ─────────────────────────────────────────────────────────
@app.errorhandler(SQLAlchemyError)
def handle_sqlalchemy_error(e):
    print("[SQLAlchemyError]", e)  # 터미널 로그
...
def admin():
    ...
    pending_orders = []
    for o in db.query(Order).filter_by(status="pending").order_by(Order.id.desc()).all():
    ...
    paid_orders = []
    for o in db.query(Order).filter_by(status="paid").order_by(Order.id.desc()).all():
        items = [{
    ...
    completed_orders = []
    for o in db.query(Order).filter_by(status="completed").order_by(Order.id.desc()).all():
    ...
    sales_sum = db.query(text(
        "SUM(totalPrice)"
    )).filter(text(
        "service=0 AND (status='paid' OR status='completed')"
    )).scalar() or 0
...
def admin_deliver_item(order_id, menu_name):
    ...
    try:
        ...
        item = next((it for it in o.items if it.menu.name == menu_name), None)
        if not item:
            flash("해당 메뉴가 주문에 없습니다.", "error")
            return redirect(url_for("admin"))

        if item.deliveredQuantity < item.doneQuantity:
...
def admin_log_page():
    ...
    return render_template("admin_log.html", logs=[{
        "time": l.time,
        "role": l.role,
        "action": l.action,
        "detail": l.detail
...
def admin_service():
    ...
    try:
        ...
        log_action(session["role"], "ADMIN_SERVICE",
                   f"주문ID={o.id} /메뉴:{menu_name}/{qty}")
        flash(f"0원 서비스 주문이 등록되었습니다. (주문 {o.id})")
    except SQLAlchemyError as e:
        db.rollback()
        print("[SQLAlchemyError]", e)
...
def kitchen():
    ...
    for o in paid:
        ...
        for it in o.items:
            ...
            if left > 0:
                kitchen_status[name] = kitchen_status.get(name, 0) + left
        paid_orders.append({
            "id": o.id,
            "order_id": o.order_id,
            "tableNumber": o.tableNumber,
